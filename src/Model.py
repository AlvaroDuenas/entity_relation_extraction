...
class Model(BertPreTrainedModel):
    def __init__(config: BertConfig, cls_token: int,
                 relation_types: int, entity_types: int,
    ...
    def train(self, encodings: torch.tensor, context_masks: torch.tensor,
        ...
        def _classify_entities(self, encodings, h, entity_masks, size_embeddings):
            ...
            entity_spans_pool = m + \
                h.unsqueeze(1).repeat(1, entity_masks.shape[1], 1, 1)
    ...
    def _classify_relations(self, entity_spans, size_embeddings, relations, rel_masks, h, chunk_start):
        ...
        rel_repr = torch.cat(
            [rel_ctx, entity_pairs, size_pair_embeddings], dim=2)
    ...
    def _filter_spans(self, entity_clf, entity_spans, entity_sample_masks, ctx_size):
        ...
        for i in range(batch_size):
            ...
            if not rels:
                ...
                batch_rel_masks.append(torch.tensor(
                    [[0] * ctx_size], dtype=torch.bool))
        ...
        batch_rel_sample_masks = util.padded_stack(
            batch_rel_sample_masks).to(device)
...
class Loss:
    ...
    def compute(self, entity_logits, rel_logits, entity_types,
                rel_types, entity_sample_masks, rel_sample_masks):
        ...
        entity_loss = (entity_loss * entity_sample_masks).sum() / \
            entity_sample_masks.sum()
